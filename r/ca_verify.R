...
#'   from the data range.
#' @param threads number of threads (effective only when the package is
#'   compiled with OpenMP; otherwise runs single-threaded). Default 1.
...
#' ca_verify(ca[-1, ], t = 2)
#' @export
ca_verify <- function(x, t, v = NULL, threads = 1L, report = 10L) {
    ...
    res <- .Call(C_ca_verify, x, t, v, as.integer(threads), as.integer(report))
